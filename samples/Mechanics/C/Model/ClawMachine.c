...
BOOL State0_Region1_Reset( ClawMachine* pClawMachine, State0_Region1* pStm, HdStateMachine* pParentStm, uint64_t nEntryPoint ) {
    if( pParentStm != NULL ){
...
BOOL ClawMachineStm_Reset( ClawMachine* pClawMachine, ClawMachineStm* pStm, HdStateMachine* pParentStm, uint64_t nEntryPoint ) {
    if( pParentStm != NULL ){
        pStm->base.pParentStm = pParentStm;
    }
    if( nEntryPoint == STATE_UNDEF ){
...
BOOL ClawMachine_EventProc( ClawMachine* pClawMachine, ClawMachine_EVENT nEventId, void* pEventParams ){
    BOOL bResult = ClawMachineStm_EventProc( pClawMachine, &pClawMachine->mainStm, nEventId, pEventParams );
    do{}while( ClawMachineStm_StateDefaultTrans( pClawMachine, &pClawMachine->mainStm ) );
    return bResult;
}
