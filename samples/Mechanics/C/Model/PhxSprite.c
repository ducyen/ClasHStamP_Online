...
static bool PhxSprite_load(
    ...
    double angle_degrees = -pPhxSprite->m_angle;
    double angle_radians = angle_degrees * (M_PI / 180.0);

    // Create a rotation vector from the angle
    cpVect rotation_vector = cpvforangle(angle_radians);

    // Draw the polygon on the texture
    for (int i = 0; i < pPhxSprite->m_vertsCnt; i++) {
        cpVect v1 = cpvrotate( pPhxSprite->m_verts[i], rotation_vector);
        cpVect v2 = cpvrotate( pPhxSprite->m_verts[(i + 1) % pPhxSprite->m_vertsCnt], rotation_vector);
    ...
    while (current != NULL) {
        current->shape = cpSpaceAddShape(space, cpPolyShapeNew(pPhxSprite->m_body, current->vertexCount, current->vertices, cpTransformRotate(-pPhxSprite->m_angle * M_PI / 180.0), 0.0));
...
Sprite* PhxSprite_Copy( PhxSprite* pPhxSprite, const PhxSprite* pSource ){
    ...
    pPhxSprite->m_angle = pSource->m_angle;
